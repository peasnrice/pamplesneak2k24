...
function updateSneakIndicator(sneaksSubmitted) {
    $('.sneak-indicator').each(function (index) {
        $(this).removeClass('sneak-active sneak-completed');
        if (index < sneaksSubmitted) {
            $(this).addClass('sneak-completed');
        } else if (index === sneaksSubmitted) {
            $(this).addClass('sneak-active');
        }
    });
}